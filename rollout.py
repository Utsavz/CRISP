...
class RolloutWorkerOriginal:
    ...
    def __init__(self, make_env, policyList, dims, logger, hrl_imitation_loss, num_upper_demos, adversarial_loss, T, rollout_batch_size=1,
                 ...
                 random_eps=0, history_len=100, render=False, sac=False, predictor_loss=0, populate=0, is_multiple_env=False, hac=0, dac=0, **kwargs):
    ...
    def generate_rollouts(self, seed=0, epoch_num=0, is_train=False):
        ...
        if self.populate and is_train and epoch_num % populate_num == 0 and self.num_hrl_layers > 1:
            ...
            '''
            if 'Rope' in str(self.envs[0]):
                self.populate_upper_demo_buffer_rope(self.policyList[0])
            elif 'Pick' in str(self.envs[0]):
                self.populate_upper_demo_buffer_pick(self.policyList[0])
            elif 'Maze' in str(self.envs[0]):
                self.populate_upper_demo_buffer_maze(self.policyList[0])
            elif 'kitchen' in str(self.envs[0]):
                self.populate_upper_demo_buffer_kitchen(self.policyList[0])
    ...
    def populate_upper_demo_buffer_maze(self, policy):
        ...
        for eps_index in tqdm(range(num_episodes)):
            import time
            start = time.time()
            ...
            demo_episodes_obs = demoData_obs
            goal = demo_episodes_obs[0].get('desired_goal')
            transition = 0
            episode_len = len(demo_episodes_obs)
            prev_transition = 0
            old_transition = -1
            outer_old_transition = -1
            transition_len = 1
            while transition < episode_len-1:
                if outer_old_transition > self.env_populate._max_episode_steps:
                    break
                initial_obs = demo_episodes_obs[transition].copy()
                prev_transition = transition
                block_reward = 0
                curr_obs = initial_obs.get('observation').reshape(1,-1)
                curr_ag = initial_obs.get('achieved_goal').reshape(1,-1)
                timestep_len = 0
                while block_reward == 0 and transition < episode_len-1:
                    self.env_populate.env._set_state(initial_obs.get('observation'), goal)
                    curr_obs = np.array(self.env_populate.env._get_obs()['achieved_goal']).copy()
                    transition += transition_len
                    transition = min(transition, episode_len-1)
                    goal_lower = demo_episodes_obs[transition].get('achieved_goal').reshape(1,-1)
                    for i in range(15):
                        outer_old_transition += 1
                        if curr_obs.ndim > 1:
                            curr_obs = curr_obs[0]
                        obs_temp = np.array(curr_obs[:3].tolist() + maze_array.copy())
                        policy_output = policy.get_actions_sac(obs_temp.copy(), curr_ag[0], goal_lower)
                        if policy_output.ndim == 1:
                            policy_output = policy_output.reshape(1, -1)
                        if 'Rope' in str(self.envs[0]):
                            env_obs, _, _, info = self.env_populate.step_rope(policy_output[0])
                        else:
                            env_obs, _, _, info = self.env_populate.step_maze(policy_output[0])
                        curr_obs = env_obs.get('observation')[:3].reshape(1,-1)
                        curr_ag = env_obs.get('achieved_goal').reshape(1,-1)
                        block_reward = self.env_populate.env.compute_reward(curr_ag, goal_lower, reward_type='sparse')
                        if block_reward == 0:
                            break
                    goal_reward = self.env_populate.env.compute_reward(curr_ag, goal.reshape(1,-1), reward_type='sparse')
                    if goal_reward == 0:
                        break
                goal_lower = demo_episodes_obs[transition].get('achieved_goal').reshape(1,-1)
                self.env_populate.env.set_subgoal('subgoal_2', goal_lower[0])
                # env.render()
                action_temp = (goal_lower[0] - action_offset) / [x for x in max_u]
                action = np.zeros(4)
                action[:3] = action_temp

                for current_transition in range(prev_transition, transition):
                    for next_transition in range(current_transition+1, transition):
                        episodeObs = []
                        episodeAcs = []
                        demoData_obs[current_transition]['achieved_goal'] = goal_lower[0].copy()
                        episodeObs.append(demoData_obs[current_transition])
                        episodeAcs.append(np.array([action]))
                        demoData_obs[next_transition]['achieved_goal'] = goal_lower[0].copy()
                        episodeObs.append(demoData_obs[next_transition])
                        actions.append(episodeAcs)
                        observations.append(np.array([episodeObs]))

                goal_reward = self.env_populate.env.compute_reward(curr_ag, goal.reshape(1,-1))
                if goal_reward == 0:
                    break
            # Add last goal
            goal_lower = demo_episodes_obs[-1].get('desired_goal').reshape(1,-1)
            action_temp = (goal_lower[0] - action_offset) / [x for x in max_u]
            action = np.zeros(4)
            action[:3] = action_temp
            for current_transition in range(prev_transition, episode_len):
                for next_transition in range(current_transition+1, episode_len):
                    episodeObs = []
                    episodeAcs = []
                    demoData_obs[current_transition]['achieved_goal'] = goal_lower[0].copy()
                    episodeObs.append(demoData_obs[current_transition])
    ...
    def populate_upper_demo_buffer_pick(self, policy):
        ...
        actions = []#[[] for i in range(num_episodes)]
        policy.clear_buffer_upper()
        self.env_populate._max_episode_steps = 50
        max_u = [0.24, 0.27, 0.145]
        action_offset = [1.29, 0.75, 0.555]
        self.env_populate.reset()
        for eps_index in tqdm(range(num_episodes)):
            ...
            while transition < episode_len-1:
                ...
                while block_reward == 0 and transition < episode_len-1:
                    self.env_populate.env._set_state(initial_obs.get('observation'), goal)
                    curr_obs = np.array(self.env_populate.env._get_obs()['observation']).copy()
                    transition += transition_len
                    transition = min(transition, episode_len-1)
                    goal_lower = demo_episodes_obs[transition].get('achieved_goal').reshape(1,-1)
                    for i in range(15):
                        ...
                        if curr_obs.ndim > 1:
                            curr_obs = curr_obs[0]
                        policy_output = policy.get_actions_sac(curr_obs.copy(), curr_ag[0], goal_lower)
                        if policy_output.ndim == 1:
                            policy_output = policy_output.reshape(1, -1)
                        if 'Rope' in str(self.envs[0]):
                            env_obs, _, _, info = self.env_populate.step_rope(policy_output[0])
    ...
    def populate_upper_demo_buffer_rope(self, policy):
        ...
        for eps_index in tqdm(range(num_episodes)):
            ...
            demo_episodes_obs = demoData_obs
            demo_episodes_states = demoData_states
            goal = demo_episodes_obs[-1].get('desired_goal')
            transition = 0
            episode_len = len(demo_episodes_obs)
            # print(episode_len)
            prev_transition = 0
            ...
            outer_old_transition = -1
            initial_obs = demo_episodes_obs[transition].copy()
            initial_state = demo_episodes_states[transition].copy()
            append_count = 0
            max_timestep_allowed = 5
            transition_len = 1
            while transition < episode_len-1:
                max_timestep_allowed = 5
                if outer_old_transition > self.env_populate._max_episode_steps:
                ...
                while block_reward == 0 and transition < episode_len-1:
                    max_timestep_allowed -= 1
                    self.env_populate.env._set_state(initial_state, goal)
                    curr_obs = np.array([self.env_populate.env._get_obs()['achieved_goal']]).copy()
                    transition += transition_len
                    transition = min(transition, episode_len-1)
                    goal_lower = demo_episodes_obs[transition].get('achieved_goal').reshape(1,-1)
                    for i in range(5):
                        ...
                        if curr_obs.ndim > 1:
                            curr_obs = curr_obs[0]
                        obs_temp = np.array(curr_obs)
                        policy_output = policy.get_actions_sac(obs_temp.copy(), curr_ag, goal_lower)
                        if policy_output.ndim == 1:
                            policy_output = policy_output.reshape(1, -1)
                        for num in range(5,20):
                            self.env_populate.env.set_subgoal('subgoal_'+str(num), np.array([goal_lower[0][2*(num-5)], goal_lower[0][2*(num-5)+1], 0.42]))
                        if 'Rope' in str(self.envs[0]):
                            env_obs, _, _, info = self.env_populate.step_rope(policy_output[0])
                        else:
                            env_obs, _, _, info = self.env_populate.step_maze(policy_output[0])
                        curr_obs = env_obs.get('observation').reshape(1,-1)
                        curr_ag = env_obs.get('achieved_goal').reshape(1,-1)
                        curr_state = self.env_populate.env._get_state()
                        block_reward = self.env_populate.env.compute_reward(curr_ag, goal_lower)
                        if block_reward == 0:
                            break
                    goal_reward = self.env_populate.env.compute_reward(curr_ag, goal.reshape(1,-1))
                    if goal_reward == 0 or max_timestep_allowed <= 0:
                        break
                action_temp = (goal_lower[0] - action_offset) / [x for x in max_u]
                action = action_temp.copy()
                for i in range(prev_transition, transition):
                    initial_obs = demo_episodes_obs[i].copy()
                    initial_state = demo_episodes_states[i].copy()
                    initial_obs['achieved_goal'] = goal_lower[0].copy()
                    episodeObs.append(initial_obs)
                    episodeAcs.append(action)
                    episodeStates.append(initial_state)
    ...
    def populate_upper_demo_buffer_kitchen(self, policy):
        ''' Populates upper level demo buffer in franka kitchen environment
         '''
        ...
        for eps_index in range(num_episodes):
            ...
            goal = demo_episodes_obs[-1].get('desired_goal')
            transition = 0
            episode_len = len(demo_episodes_obs)-1
            prev_transition = 0
            old_transition = -1
            outer_old_transition = -1
            initial_obs = demo_episodes_obs[transition].copy()
            ...
            max_timestep_allowed = 5
            transition_len = 1
            goal_lower_previous = demo_episodes_obs[0].get('achieved_goal').reshape(1,-1)
            while transition < episode_len-1:
                max_timestep_allowed = 5
                if outer_old_transition > self.env_populate._max_episode_steps:
                    break
                ...
                while block_reward == 0 and transition < episode_len-1:
                    max_timestep_allowed -= 1
                    self.env_populate.env._set_state(initial_obs.get('observation'), goal)
                    curr_obs = np.array([self.env_populate.env._get_obs_real()['observation']]).copy()
                    transition += transition_len
                    transition = min(transition, episode_len-1)
                    goal_lower = demo_episodes_obs[transition].get('achieved_goal').reshape(1,-1)
                    if not np.any(np.round(goal_lower[0], decimals = 1)) or np.max(np.abs(np.round(goal_lower[0], decimals = 1))) < 0.5 or np.max(np.abs(np.abs(np.round(goal_lower[0], decimals = 2)) - np.abs(np.round(goal_lower_previous[0], decimals = 2)))) < 0.4:
                        continue
                    for i in range(17):
                        outer_old_transition += 1
                        if curr_obs.ndim > 1:
                        ...
                        if policy_output.ndim == 1:
                            policy_output = policy_output.reshape(1, -1)
                        env_obs, _, _, info = self.env_populate.step(policy_output[0])
                        curr_obs = env_obs.get('observation').reshape(1,-1)
                        curr_ag = env_obs.get('achieved_goal').reshape(1,-1)
                        block_reward = self.env_populate.env.compute_reward(curr_ag, goal_lower)
                        if block_reward == 0:
                    ...
                    if goal_reward == 0:# or max_timestep_allowed <= 0:
                        break
                goal_lower_previous = goal_lower.reshape(1,-1).copy()
                goal_lower_1 = (goal_lower[0] - action_offset) / [x for x in max_u]
                ...
                action = action_temp.copy()
                for current_transition in range(prev_transition, transition):
                    if current_transition+1 == transition and current_transition+1 < episode_len:
                        next_transition = current_transition+1
                        episodeObs = []
                        episodeAcs = []
                        episodeObs.append(demo_episodes_obs[current_transition])
                        episodeAcs.append(np.array([action]))
                        episodeObs.append(demo_episodes_obs[next_transition])
                        actions.append(episodeAcs)
                        observations.append(np.array([episodeObs]))
                    else:
                        for next_transition in range(current_transition+1, transition):
        ...
        if hrl_layer_num > 0:
            ...
            for t in range(self.T):
                ...
                for i in range(self.rollout_batch_size):
                    try:
                        ...
                        curr_obs, curr_ag, lowest_ag, info, num_lwr_eps_collisions_with_wall = self.rec_rollout_generator(hrl_layer_num - 1, seed)
            ...
            for t in range(self.T):
                ...
                if self.raps:
                    if 'Rope' in str(self.envs[0]):
                        ...
                        policy_output = self.goal_array[hrl_layer_num][0][:29].reshape(1,-1).copy()
                elif 'Rope' in str(self.envs[0]):
            ...
            if self.testingNow != 1:
                ...
                if optimalFlag:
                    ...
                    self.obs_final, self.achieved_goals_final, self.acts_final, self.goals_final, self.successes_final, self.rewards_final, self.env_indexes_final = [], [], [], [], [], [], []
            return hrl_obs.copy(), hrl_ag.copy(), hrl_ag.copy(), infoG, num_eps_collisions_with_wall
